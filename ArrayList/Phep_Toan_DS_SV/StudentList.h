...
// Luu y tren else khai bao sinh vien bang typedef
DanhSach dsRong(){
    DanhSach L;
    L.n=0;
    return L;
...
void hienthi(DanhSach L){
    ...
    while(p!=L.n+1){
        s=L.A[p-1];
        printf("%s - %s - %.2f - %.2f - %.2f - %.2f\n",s.MSSV, s.HoTen, s.DiemLT, s.DiemTH1, s.DiemTH2, s.DiemLT+s.DiemTH1+s.DiemTH2);
        p++;
    }
}
...
void hienthiDat(DanhSach L){
    ...
    while (p!=L.n+1){
        ...
        if (tdiem>=4){
            printf("%s - %s - %.2f - %.2f - %.2f - %.2f\n", L.A[p-1].MSSV, L.A[p-1].HoTen, L.A[p-1].DiemLT, L.A[p-1].DiemTH1, L.A[p-1].DiemTH2, tdiem);
        }
        p++;
    }
}
